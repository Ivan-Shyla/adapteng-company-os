...
|---|---|---|
| Overall stage | **Operational foundation + controlled migration.** Company-owned Baserow, the Shared Drive folder skeleton, Postgres, internal Baserow adapter and two governed self-hosted workflows are live. The exact-three `ART-2026-001` / `SRC-2026-001` approved-source import foundation is repository-merged, but no governed Drive file import runtime or business AI is live. Repository readiness is not live readiness. Re-verified read-only 2026-08-05: automation-platform rollout PRs #93, #94 and #98 are merged, with current automation `main` at `d06bdd41964e57d9fc7f1b2490d6dcde64b0143d`; no `Migrate Approved Assets` dispatch has run. The three approved-assets Environments remain restricted to `main`, but their exact v4 scope matrices are incomplete, no release-label runner is registered, and the separately required `company-os-vertex-runtime-readiness` Environment does not exist. | Complete and independently review fresh production-backup/retention plus isolated-restore/rehearsal evidence; configure the exact Environment matrices; provision the documented trusted POSIX lifecycle host and one-job private-network runner; then authorize the five migration/import phases separately and prove zero-duplicate replay. Separately establish dedicated Vertex service-account readiness before the first model proof. Keep the `CASE-2026-001` media lane blocked. |
...
| AI agent | **REJECT_LIVE; deterministic and repository runtime components are merged, but runtime prerequisites are not ready (read-only verification 2026-08-05).** Control-plane `main` is `327fc4b63ec60afc8a8a6c3169d062a58d9eb4da`; automation-platform PR #98 merged the first-model-proof runtime as `d06bdd41964e57d9fc7f1b2490d6dcde64b0143d`. No live model call has occurred. The exact proof input remains `ART-2026-001` / `SRC-2026-001`, separate from blocked CASE media. The AI Gateway migrations remain unapplied, the approved-source import/replay has not run, the dedicated `company-os-vertex-runtime-readiness` Environment and Vertex identity configuration are absent, and no local proof-runtime configuration is present. | First complete the separately governed approved-source migration/import/replay. Then establish the dedicated EU Vertex service account, IAM/API/model-metadata readiness and privacy/retention attestation; apply only the exact first-proof migrations through their own backup/rehearsal path; run the zero-write proof preflight; and permit exactly one capped `generateContent` call only after every prerequisite independently verifies. |
