...
| `drive-folder-usage-notes` | Google Drive + automation | Add one versioned `START HERE` usage note to every canonical work area and generated `AE-CAS`/`AE-CGR` folder | Repository contract approved in §4.5; **planner/apply capability now repository-merged** via automation-platform PR #91 (`427dd7f5ce87a5a72314c8169cca708300b62ea0`, 2026-07-30), but live placement is still separate and has not occurred. Prioritize `01_Inbox`, `30_Projects_Cases`, then `40_Content`. Notes cover purpose, input rules, naming/metadata, one placeholder-only example, automation/trigger/actions/output, approvals/PII and owner/version; updates are idempotent, duplicate-free, secret/assigned-ID/PII-free and preserve all human content outside managed sections. |
...
| `MKT-002` | marketing + automation | Connect article flow to `Content_Items` and Drive | **Approved-source import foundation repository-merged, not live:** automation-platform PR #89 was independently reviewed and squash-merged as `dbcf806ea7714b8e2a7415ae6cd788491924178d` (reviewed/merged tree `8fc649a97963f38ce0a7592001a7fea4834eceea`). Its bounded workflow admits exactly three immutable Git-byte JSON objects for `ART-2026-001` / `SRC-2026-001`, uses the allocator-returned `AE-CGR` identity, performs sealed Drive upload, verifies exactly 2 folders / 3 files on replay, and emits boolean/count-only receipts. It creates no Baserow row and excludes AI/model calls, publication, media binaries and extra historical files. Migrations `007_source_identity_reservation.sql` and `008_drive_bridge_replay_reservations.sql` are merged but unapplied; unrelated `008_ai_gateway_runtime_hardening.sql` is forbidden for this workflow. PR #89 triggered no dispatch or live write. **Rollout scaffolding has since progressed (re-verified read-only 2026-08-03):** all three GitHub Environments now exist and `approved-assets-preflight`/`approved-assets-import` are configured with the required `GOOGLE_CLOUD_PROJECT`/`GOOGLE_DRIVE_SCOPE`/`GOOGLE_WORKSPACE_ADMIN` variables and Drive/DB secrets (names only); PR #90 added a machine-checkable readiness CLI and PR #92 pinned an exact self-hosted runner label set while explicitly stating rollout readiness is still not claimed pending an Enterprise-Required-Reviewers substitute. Four exact-subject rollout-authorization PRs (`#93`–`#95`, `#97`) remain open/unmerged. No `Migrate Approved Assets` workflow run has ever executed — dispatch remains zero. |
...
| `AG-008` | ai-dev-loop-control-plane | Harden production completion boundary | **Repository implementation merged, not independently re-reviewed:** PR #38 (`c6a5b5090acb2712ed2fb388a3170f9053f417b6`, merged 2026-07-30) requires a schema-valid `BusinessTaskEnvelope`, enforces `no_external_action: true` with recursive rejection of approval/publish/send/action/execute fields and IDs, and makes `ModelGateway` cost accounting atomic and cap-bounded, closing all four failures the 2026-07-26 audit reproduced (211/446 tests passing). PR #39 (`edadb09125f7fb5d173d5f595181d1384050b6b5`, merged 2026-08-02) then closed a related gap in the general JSON validator (duplicate keys / non-finite constants in `LOOP_STATE.json`, project profiles and reviewer verdicts), which the PR itself flags as the same class of audit-integrity defect. `agent/NEXT_TASK.md` self-declares `status: done`; CI on `edadb091` is green. No separate third-party review of this exact head is recorded, and this closes only the deterministic control-plane half — automation-platform's persistent Postgres cost reservation and EU Vertex adapter remain unbuilt/undeployed. |
...
| ID | Repository/system | Work | Definition of done |
|---|---|---|---|
| `WEB-001` | website + automation | Versioned `lead.created` contract | Source, consent, language, service and correlation ID. **Repository-merged, not live:** re-verified read-only 2026-08-03, PR #78 (`feat(lead-intake): versioned lead.created v1 contract with PII separation (WEB-001)`) merged 2026-08-01T22:54:57Z as `6770e749fbf1345bfe15f260e574da93fa4df329`, superseding the historical `b0e3a656`/`1baedaf73`/`a23b194f` draft heads. It adds `docs/contracts/lead.created/lead.created.v1.schema.json`, privacy-notice translations, validator/mock tooling and a WordPress producer plugin (`wp-content/plugins/adapteng-core/includes/lead-intake.php`). This is repository merge only: the manual, confirmation-phrase-gated `Deploy to Cloudways` plugin workflow that ships `adapteng-core` has run repeatedly, but every recorded run (last success `30720691975`, 2026-08-01T22:10:23Z) predates this merge, so no plugin-deploy run is evidenced to carry the new lead-intake code — it is not shown live. No review-clean or cutover-ready claim is made. Require an exact-head independent review-clean result on `6770e749`, a plugin-deploy dispatch that actually includes this code, actual producer T1-T4, atomic no-dual-write cutover, rollback proof and reconciliation evidence. |
...
| Component | Verified status (through 2026-08-10 unless a row cites a later date) | Next milestone / constraint |
|---|---|---|
| Overall stage | **Operational foundation + controlled migration.** Company-owned Baserow, the Shared Drive folder skeleton, Postgres, internal Baserow adapter and two governed self-hosted workflows are live. The exact-three `ART-2026-001` / `SRC-2026-001` approved-source import foundation is repository-merged, but no governed Drive file import runtime or business AI is live. Repository readiness is not live readiness. Owner-reconciled 2026-08-06: the real production PostgreSQL backup, off-provider Backblaze availability, isolated restore, restored PostgreSQL startup/readability and backup/restore evidence are accepted and **CLOSED**; stale `NOT_CONFIGURED` backup/rehearsal status must not reopen or block this rollout. Automation PRs #93, #94, #98 and #104 are merged, with current automation `main` at `0794212ea74a45095a45b9e1046cf5f8dc89f946`; no `Migrate Approved Assets` dispatch or live model call has run. The approved-assets Environments remain restricted to `main`; their persistent content-read credentials are configured, but migration evidence and short-lived per-phase authorization still require the governed lifecycle, and no release-label runner is registered. The exact `company-os-vertex-runtime-readiness` Environment exists, is restricted to `main`, and contains the three required secret names. Reconciled 2026-08-10: **all nine logical migration units are applied in production and must not be replayed** (owner's post-rollout manual production check), the automation evidence-lifecycle dependency is **CLOSED** via automation PRs #93/#94/#98, and the `ai-gateway` is **implemented and tested, not deployed** (run `31214858400`). | Provision only the documented one-job private-network rollout identity on the existing host if the runtime gate requires it; then authorize the remaining import phases separately and prove zero-duplicate replay. Run Vertex readiness through the owner-dispatch fallback before the first model proof. Keep the `CASE-2026-001` media lane blocked. |
| Weekly repository delta | **Merged, not live:** automation `main` advanced after the 2026-08-06 status through PRs #105 (`a19c9fdb30122c3e94cc40adc95a78cc94d64978`), #106 (`8441ad5b39102ef68b1a66f98380a8cfb262e27a`), #107 (`d6ab6322983af42e355dedea4de6d0d21752de59`) and #108 (`23a23f0fd5cacd630badf3bd20503789366f7220`). The first three add digest-pinned, fail-closed `status/apply` runners and prerequisite gates for all nine logical migration units; #108 schema-qualifies allocator access as `public.id_allocator_sequences`. This supersedes the older automation-main SHA in the Overall row, but it does not evidence a dispatch, migration apply, deployment or model call. Control-plane and marketing have no post-baseline repository delta; no actual-cost evidence changed. | Use the runners only through the existing governed authorization lifecycle. Treat successful CI as repository evidence, not rollout acceptance. |
...
| Drive folder usage notes | **Repository implementation merged, not live:** automation-platform PR #91 (`427dd7f5ce87a5a72314c8169cca708300b62ea0`, merged 2026-07-30) pins the `START HERE` contract to immutable commit/tree/blob SHA-256 evidence and adds typed, allowlisted planning plus a sealed apply path for the eight canonical work areas and discovered `AE-CAS-*`/`AE-CGR-*` folders (288 passed/5 skipped service-level, 415 passed/7 skipped/368 subtests repo-wide). It preserves human-authored bytes outside managed markers and fails closed on ambiguity/collision/truncation. No workflow, credential, dispatch or live apply was added or run; no note has been placed live. | Implement and approve live placement separately using this merged planner/apply capability. Start with `01_Inbox`, `30_Projects_Cases`, then `40_Content`; prove idempotent create/update, no duplicates/secrets/assigned IDs/PII and preservation of human-authored content outside managed sections. |
| Drive implementation | Automation-platform PR #89 is independently reviewed and repository-merged at `dbcf806ea7714b8e2a7415ae6cd788491924178d`; its merge tree exactly matches reviewed tree `8fc649a97963f38ce0a7592001a7fea4834eceea`. The bounded approved-source path admits only the exact immutable three-JSON `ART-2026-001` package, takes its `AE-CGR` identity dynamically from the allocator, seals the Drive upload, verifies exactly 2 folders / 3 files on replay, and emits boolean/count-only receipts. Baserow rows, AI generation/model calls, publication, media binaries and extra historical files are outside scope. The exact-subject authorization chain is now merged: PR #93 as `1f420dc0f1cc7cfc88fa8037e00b982c0514cc08`, PR #94 as `0fa357d0baebc362b5bea0afba78e6233d91b7c8`, and PR #98 as `d06bdd41964e57d9fc7f1b2490d6dcde64b0143d`. Read-only verification on 2026-08-05 found zero workflow runs and zero registered rollout runners. All three approved-assets Environments are `main`-only, but their v4 matrices are incomplete: the short-lived authorization/review packets are correctly absent between phases, while the persistent source-read access entries and migration evidence required before rollout are also absent. | Do not dispatch even `db_status` until a fresh reviewed v4 evidence packet exists and the complete matrix passes the offline readiness gate. Run the four safe phases (`db_status`, `preflight`, `import`, `replay_verify`) only from the documented trusted POSIX lifecycle host with a fresh run-specific private-network runner and fresh exact-subject authorization for each phase. Never run the legacy `apply_required_migrations` phase: all nine migration units are already exact in production. |
...
| Postgres `adapteng_ops` | Live, internal-only PostgreSQL 16 in Coolify resource `adapteng-ops-db` on Hetzner local Docker storage. **ALL NINE LOGICAL MIGRATION UNITS ARE APPLIED IN PRODUCTION — DO NOT REPLAY THEM.** Corrected 2026-08-10: this row previously read "002/003/005/006 plus `007_source_identity_reservation.sql` and `008_drive_bridge_replay_reservations.sql` are merged but unapplied (`live:false`)" and named 007/Drive-008 as the migrations permitted for the approved-source import. The owner's post-rollout manual production check found 001, 002, 003, 004, 005, 006, 007, `008_drive_bridge_replay_reservations.sql` and `008_ai_gateway_runtime_hardening.sql` all exact in production, and production outranks a repository note. The check is owner-attested and not reproducible from GitHub: the `Migrate Approved Assets` workflow has **zero** runs (verified read-only 2026-08-10), so the rollout left no dispatch record — run-count zero is **not** evidence of an unapplied database, and reading it that way is the error corrected here. `UNVERIFIED`: per-unit applied state and rollout timestamp; a read-only `db_status` or catalog capture attached to a status PR would settle it. Fail-closed status/apply runners for all nine units are merged and CI-green (automation PRs #105/#106/#107), but a merged runner is a capability, not an instruction to run it. Migration 001's allocator schema mismatch is closed in code by automation PR #108 (`23a23f0fd5cacd630badf3bd20503789366f7220`, merged 2026-08-08, five checks green), which pins bootstrap and runtime access to `public.id_allocator_sequences`; PR #108 states it made no production change, so the live disposition of the misplaced `adapteng_ops` copy is `UNVERIFIED`. The selected design is operator-managed pinned pgBackRest 2.59.0 physical base backup + continuous WAL using provider-managed private B2 EU Central object storage, restored on independent clean disposable PostgreSQL-only Hetzner hosts. **It is documented and, as of 2026-08-03, rehearsed end to end on disposable infrastructure — but it is not configured for production, and is not provider-managed PostgreSQL backup.** The object store it targets is, however, now proven reachable: dispatch-only run `30752237109` (2026-08-02, head `7f5f3585588da8b330e4ae9779f0b6343e1156eb`, conclusion `success`) wrote a probe object to the private B2 EU Central bucket, read it back and compared bytes, deleted it, and asserted with `head-object` that it was gone — so the configured credentials authenticate and deletion is effective, meaning bucket Object Lock is not silently making retention unenforceable. That run proved connectivity and nothing more. The contour itself was proven separately on 2026-08-03 by dispatch-only run `30811383748` (head `1f270cfb668c2ffaddd95f21db90ae17c811ef9a`, conclusion `success`, 23 of 23 steps with none skipped and none failed): it installed PostgreSQL 16 and pgBackRest, created a stanza, took a real client-side-encrypted full backup to the same private bucket (`full backup size = 29.3MB, file total = 989`, landing as 14 objects totalling 9,437,040 bytes), ran `verify`, archived WAL, destroyed the source cluster so restored bytes could only have come out of B2, restored once to the pre-backup state (identical across 4 tables and 45,001 rows), restored again to the end of archived WAL (identical across 5 tables and 46,203 rows), then removed the rehearsal prefix and asserted it empty. A negative control in the same job correctly reported the restored state differing from the deliberately-changed source in 6 ways, so the comparison instrument was demonstrated capable of returning a difference and the identity results are not vacuous. **That is the contour, not production:** the rehearsal database is disposable and synthetically seeded, 65 isolation assertions proved production was unreachable and the run-scoped prefix shared no path lineage with the production repository path, and `adapteng_ops` itself still has no stanza, no schedule, no retention enforcement and no monitoring — a passing rehearsal proves the mechanism, not that production is recoverable. The non-secret repository configuration lives in the `PGBACKREST_REPO1_*` repository variables (values deliberately not in Git), which are the single source of truth for bucket, endpoint, region, key type, URI style, repository path and cipher type. Tracked restore code generates exact public config, accepts secrets only through fixed descriptors, carries authoritative target IDs in private state, completely validates each never-started target before its sole ID start, measures and executes one runner ID, and uses fresh one-use challenges through a pinned owner-side signed provider broker to prove exact firewall plus empty private-network state before target start/SQL and after cleanup. Its retention exporter now declares a dedicated-host scope, verifies no user managers/linger, and inventories all system/per-user scheduler sources plus every root/admin/writer-capable process; the current shared Coolify scope is therefore not accepted without a separately reviewed exclusive-broker replacement. Immutable image/runner/provider/exporter manifests remain unconfigured; no artifact has touched Docker or a provider. Coolify's `pg_dump`, a manual logical dump, the Baserow all-in-one backup and a running-host snapshot do not satisfy the contract. | **Migrations: applied — do not replay.** Backup: `BLOCKED_ON_UNCONFIGURED_PRODUCTION_BACKUP`. No backup rollout until separately reviewed immutable source/derived/runner/provider/exporter manifests, dedicated-host or exclusive-broker scope, exact runtime/catalog compatibility, fresh selected full with post-backup `check`, parsed `verify` status `ok` and WAL health, canonical accepted 12-full retention packet, fresh authorization inventory, complete provider-cost proof, independent-host A pre-migration restore, B exact sealed apply plus descriptor-streamed DML rollback with zero durable synthetic state, independent-host C ending in B's exact migrated catalog state, cleanup, and reviewed digest-only `PASS` exist. The automation evidence-lifecycle dependency that previously held this row at `NOT_READY_PENDING_AUTOMATION_EVIDENCE_LIFECYCLE_PR` is **CLOSED** — it merged as automation PRs #93 (`1f420dc0f1cc7cfc88fa8037e00b982c0514cc08`), #94 (`0fa357d0baebc362b5bea0afba78e6233d91b7c8`) and #98 (`d06bdd41964e57d9fc7f1b2490d6dcde64b0143d`) on 2026-08-05, PR #94 being the evidence-lifecycle unit covering exactly the five sanitized fields; `UNVERIFIED` is only whether a reviewed sanitized `PASS` has been produced against a real packet. Do not enable bucket Object Lock, and do not apply any migration through this workflow — all nine units are already applied. |
| Production allocator schema | **Live defect evidenced, remediation not run:** automation PR #108 records a read-only 2026-08-08 diagnosis that the structurally correct allocator table is in schema `adapteng_ops`, not canonical `public`, because the former bootstrap/runtime SQL was unqualified. PR #108 merged the repository fix and a real-Postgres regression test; it explicitly did not move or rewrite the production table. | Owner must separately approve a production remediation plan that preserves allocated sequences, proves callers use `public.id_allocator_sequences`, and includes rollback plus post-change read-only verification. Do not infer remediation from the merge. |
| Platform governance evidence | Current live authority remains the evidence in this table: Postgres is operational truth; Company Drive is artifact authority; Baserow is the human projection. Company Shared Drive and the Baserow skeleton are live; **all nine logical migration units are applied in production and must not be replayed** (owner's post-rollout manual production check; see the Postgres row); the internal adapter plus AUT-001/WEB-002 are live and proven; n8n Cloud is 89 non-archived / 31 active / 58 inactive after the reversible freeze. PR #89 adds repository readiness only and does not override live evidence. | Still open: the approved-source import gates; compromised Baserow token owner remediation; GitHub controls below; workflow-to-credential binding isolation and second-admin continuity; base vendor cost/renewal/SLA evidence; media implementation consolidation; and MCP exposure closure. **Removed 2026-08-10: "live rollout of repo-only migrations" — there are no repo-only migrations left; all nine units are applied in production and replaying them is forbidden.** |
| GitHub repository controls | **Live as of 2026-08-02T15:30 CEST, re-verified via read-only ruleset API on 2026-08-03 and Vertex Environment API on 2026-08-06:** an active `main-protected` repository ruleset exists on all five active repositories — `adapteng-company-os` (id `20236724`), `adapteng-automation-platform` (`20236725`), `ai-dev-loop-control-plane` (`20236728`), `adapteng-marketing` (`20236729`), `adapteng-website` (`20236726`) — each blocking branch deletion and non-fast-forward pushes and requiring a pull request (0 required approving reviews, no code-owner/last-push requirement, thread resolution not enforced) plus repository-specific required status checks. `current_user_can_bypass` is `never`. GitHub returned HTTP 422 when the owner attempted to add a required reviewer to the private Vertex Environment because the current plan does not support that protection. Owner decision 2026-08-06 authorizes the reviewed fallback merged by automation PR #104: only explicit `workflow_dispatch` by canonical owner `Ivan-Shyla`, exact canonical non-fork repository, protected branch/ref `main`, and a pre-secret API equality check between dispatch SHA and the current `main` tip; the accepted SHA is recorded before Environment secrets are requested. This replaces only the unavailable reviewer UI gate and does not relax identity, IAM, model, migration, preflight, evidence or one-call limits. | Treat plan support for Environment required reviewers as optional future hardening, not a rollout blocker. Add required conversation-thread resolution if desired; confirm company ownership/transfer path. Separately isolate the shared deploy key and re-verify legacy-repo protection once a non-API check is available. |
...
| Website | `adapteng.com` is live on Cloudways. Website PR #78 head `b0e3a656cf6659b893810e11a15b9f515527ab79` is historical last-reviewed evidence only; `1baedaf732088edcc3fa4e40892d23d42b140d7b` is the historical seven-issue-blocked delivery/data-race head. **The stale `a23b194f` draft head is superseded:** re-verified read-only 2026-08-03, PR #78 (`feat(lead-intake): versioned lead.created v1 contract with PII separation (WEB-001)`) merged 2026-08-01T22:54:57Z as `6770e749fbf1345bfe15f260e574da93fa4df329`, adding the `lead.created` v1 schema/docs/examples and a WordPress producer plugin (`wp-content/plugins/adapteng-core/includes/lead-intake.php`) — see the WEB-001 backlog row for scope. `main` then gained the `main-protected` ruleset (2026-08-02T15:30 CEST) and, on a **separate theme-only track**, PR #125 (`630d933f...`) recorded owner authorization of unattended production **theme** deployment behind a snapshot/rollback/smoke-test contract, refined by PRs #121–#124/#126–#130. **GitHub Actions run evidence only (not independently re-verified live by this audit):** the `Deploy theme to Cloudways` workflow's run `30766896787` (head `18767bd1...`, 2026-08-02T21:00:26Z) completed `conclusion: success` with `Snapshot`/`Deploy theme`/`Purge Breeze cache`/`Run production smoke tests` all `success`. Separately, the manual, confirmation-phrase-gated `Deploy to Cloudways` **plugin** workflow (which would ship `adapteng-core`, including the new lead-intake code) has run repeatedly, but every recorded run (last success `30720691975`, 2026-08-01T22:10:23Z) **predates** the WEB-001 merge at 22:54:57Z — so no plugin deploy run is evidenced to contain the merged lead-intake producer. No review-clean or cutover-ready claim is made for either track. Model-provider legal placeholders remain unpublished. | Independently re-verify the live theme against the CI evidence above before treating it as live-accepted. For WEB-001: require an exact-head independent review-clean result on `6770e749`, then a plugin-deploy dispatch that actually includes the lead-intake code, actual WordPress/Fluent Forms producer T1-T4, atomic no-dual-write cutover, seven-day reconciliation and rollback proof before retiring MM-18. |
| Website weekly acceptance | **Live/accepted theme evidence, not plugin acceptance:** repository-recorded post-deploy checks on 2026-08-09 supersede the earlier theme-verification gap. PRs #139 (`ed214d3886070c15082ac3bc820523c06a34171e`) and #140 (`6768430bd874fd141018ef7ef53a0450ffae7f60`) were followed by a production crawl with 0 unreachable pages, 3 redirecting internal links, 0 dead fragments, 0 hreflang failures and 0 non-200 responses across 96 sitemap URLs. After PR #142 deployed but failed its live check, corrected PR #143 (`206e24644e7b40f43ae68da0db41f4290c9a35a8`) was deployed by run `31329017343`; the recorded post-deploy check found exactly one localized description on all 18 checked commercial pages. PR #144 (`b31c4d08cf9668412bfc6ab8ae600839414d7fa8`) is merged repository logic only and will affect the next theme workflow. `adapteng-core` remains at its 2026-08-01 deployed head, so newer plugin metadata, generated `llms.txt`, WEB-001 and the lead producer are **not live**. | Keep Expertise unpublished pending reviewed editorial content and keep the `24/7 remote` claim owner-evidence-gated. Preserve the fail-closed exact-SHA WEB-002/idempotency gate before any plugin deployment; do not retire MM-18 without producer T1-T4, atomic cutover, reconciliation and rollback proof. |
...
| AI agent | **AI Gateway: implemented and tested, NOT deployed.** Corrected 2026-08-10 — this row previously read `REJECT_LIVE until the remaining runtime gates pass`, which read as cost- or defect-blocked. It is neither. The `ai-gateway` suite is green on automation-platform `main`: run [`31214858400`](https://github.com/Ivan-Shyla/adapteng-automation-platform/actions/runs/31214858400) (*AI Gateway Tests*, head `d6ab6322983af42e355dedea4de6d0d21752de59`, 2026-08-07T20:12:40Z, conclusion `success`) passed unit tests on **`ubuntu-latest` and `windows-latest`**, PostgreSQL semantics, supply-chain gates and repo validation. Its persistent cost schema — migrations 005 and `008_ai_gateway_runtime_hardening.sql` — is applied in production per the owner's post-rollout check and **must not be replayed**. The gateway is **not** blocked on cost, model pricing or FX configuration; FX is a deploy-time value and the code fails closed without it. What is missing is the deployment itself: no `ai-gateway` container exists in Coolify (declared as a not-yet-created resource in `registry/environments.yaml`), so the tested durable cost authority is not the live authority, and the real EU Vertex client, Drive adapters, orchestration and canonical approval composition are unproven against a running service. `UNVERIFIED`: any deployment — GitHub shows none. Control-plane `main` is `327fc4b63ec60afc8a8a6c3169d062a58d9eb4da`; automation-platform `main` is `0794212ea74a45095a45b9e1046cf5f8dc89f946`. No live model call has occurred. The exact proof input remains `ART-2026-001` / `SRC-2026-001`, separate from blocked CASE media. Owner authorization 2026-08-06 selects the existing exact identity `adapteng-ai-operator@adapteng-workspace-automation.iam.gserviceaccount.com` and permits the reviewed main-only owner-dispatch fallback for Vertex readiness and the eventual first live run; Environment secret values remain non-readable and environment-scoped. The readiness workflow has not yet passed, approved-source import/replay has not run, and local proof-runtime configuration has not been verified. | Run read-only Vertex IAM/model-metadata readiness for project `adapteng-workspace-automation`, location `eu`, model `gemini-3.1-flash-lite`; then deploy the `ai-gateway` service, confirm its already-applied migrations **read-only — never re-apply them**, complete the approved-source import/replay, run the zero-write proof preflight, and permit exactly one capped `generateContent` call only after every prerequisite independently verifies. |
...
| Backup/security | Fresh `adapteng_ops` backup exists (2026-07-25 13:31, 35.21 KB, Coolify + owner copy). That is a logical dump and does not satisfy the physical/WAL contract. Off-host object-store connectivity for the planned pgBackRest repository was proven on 2026-08-02 by run `30752237109` (write, byte-compared read-back, delete, verified absence), and the full backup-and-restore contour was proven on 2026-08-03 by run `30811383748` (real client-side-encrypted full backup to B2, `verify`, WAL archive, source cluster destroyed, restore to the pre-backup state and a second restore to the end of archived WAL, both byte-compared, then prefix deleted and asserted empty). Production `adapteng_ops` still has no stanza, schedule, retention enforcement or monitoring: the rehearsal cluster is disposable and synthetically seeded. Synthetic rows are closed, but the Baserow token incident is reopened because a literal value remains in Git history; current tracked files are sanitized and owner rotation is pending. | Owner revoke/rotate the compromised Baserow token and verify the old value fails; do not rewrite history. Prove restore in scratch is **done** (run `30811383748`); next configure the production `adapteng_ops` stanza, schedule and retention. Complete Workspace recovery/MFA; rotate the chat-exposed n8n management key and Coolify API token after launch work; record actual service invoices. |
