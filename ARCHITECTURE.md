...
| Component | Verified status (through 2026-07-30 unless a row cites a later date) | Next milestone / constraint |
|---|---|---|
| Overall stage | **Operational foundation + controlled migration.** Company-owned Baserow, the Shared Drive folder skeleton, Postgres, internal Baserow adapter and two governed self-hosted workflows are live. The exact-three `ART-2026-001` / `SRC-2026-001` approved-source import foundation is repository-merged through automation-platform PR #89 / `dbcf806ea7714b8e2a7415ae6cd788491924178d`, but no governed Drive file import runtime or business AI is live. Repository readiness is not live readiness. Re-verified read-only 2026-08-03: all three rollout GitHub Environments now exist and two of them carry the required Google/DB variable and secret names (values not read); a dedicated readiness CLI (PR #90) and runner-isolation policy (PR #92) merged, but four exact-subject rollout-authorization PRs remain open and no `Migrate Approved Assets` dispatch has ever run — even zero-write phases remain unsafe to dispatch. All five active repositories also gained an active `main-protected` GitHub ruleset on 2026-08-02. | Merge the open exact-subject rollout-authorization PRs; configure and accept the documented pgBackRest physical/WAL backup path (a live rehearsal dispatch on 2026-08-03 failed at `stanza-create` with HTTP 403 — narrower than the proven bucket read/write/delete connectivity); then run zero-write preflight, review expected writes, authorize the five-phase controlled run and prove zero-duplicate replay. Keep the separate `CASE-2026-001` media lane blocked. |
