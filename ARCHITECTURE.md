...
| Component | Verified status (through 2026-08-04 unless a row cites a later date) | Next milestone / constraint |
|---|---|---|
| Overall stage | **Operational foundation + controlled migration.** Company-owned Baserow, the Shared Drive folder skeleton, Postgres, internal Baserow adapter and two governed self-hosted workflows are live. The exact-three `ART-2026-001` / `SRC-2026-001` approved-source import foundation is repository-merged through automation-platform PR #89 / `dbcf806ea7714b8e2a7415ae6cd788491924178d`, but no governed Drive file import runtime or business AI is live. Repository readiness is not live readiness. Re-verified read-only 2026-08-04: all three rollout GitHub Environments now exist and two of them carry the required Google/DB variable and secret names (values not read); a dedicated readiness CLI (PR #90) and runner-isolation policy (PR #92) merged, and two of four exact-subject rollout-authorization PRs (`#95`, `#97`) have since merged — `#93` and `#94` remain open and blocked (`#93` CONFLICTING, `#94` stacked on it) — and no `Migrate Approved Assets` dispatch has ever run, so even zero-write phases remain unsafe to dispatch. The pgBackRest rehearsal's earlier 2026-08-03 HTTP 403 is resolved (a B2 account bandwidth-cap artifact, not a backup-path defect); the scheduled nightly rehearsal has since run green repeatedly, including a full 23/23-step stanza-create/backup/dual-restore proof, but production `adapteng_ops` still has no stanza, schedule or retention configured. All five active repositories also gained an active `main-protected` GitHub ruleset on 2026-08-02. | Merge/rebase the two remaining exact-subject rollout-authorization PRs (`#93`, `#94`); configure the pgBackRest physical/WAL backup path on production `adapteng_ops` itself (rehearsal is proven, production is not); then run zero-write preflight, review expected writes, authorize the five-phase controlled run and prove zero-duplicate replay. Keep the separate `CASE-2026-001` media lane blocked. |
...
| Drive implementation | Automation-platform PR #89 is independently reviewed and repository-merged at `dbcf806ea7714b8e2a7415ae6cd788491924178d`; its merge tree exactly matches reviewed tree `8fc649a97963f38ce0a7592001a7fea4834eceea`. The bounded approved-source path admits only the exact immutable three-JSON `ART-2026-001` package, takes its `AE-CGR` identity dynamically from the allocator, seals the Drive upload, verifies exactly 2 folders / 3 files on replay, and emits boolean/count-only receipts. Baserow rows, AI generation/model calls, publication, media binaries and extra historical files are outside scope. This is repository readiness only: no dispatch, migration, upload, write, model call or deployment occurred. **Rollout scaffolding has since progressed and was re-verified read-only on 2026-08-03:** PR #90 (`7aef108ef80663ade5c361fd3657e13282bfc2a6`, 2026-07-30) added a machine-checkable rollout-readiness CLI and the exact environment/secret/variable scope matrix; PR #92 (`4afc3b13668e6a07187db2ee50ee4a283833d16d`, 2026-07-30) pinned all approved-assets phases to an exact self-hosted runner label set and explicitly states this does **not** yet claim rollout readiness because GitHub Required Reviewers on environments need GitHub Enterprise Cloud, which this private repo lacks. All three GitHub Environments now exist (`approved-assets-migrations` created 2026-07-31T08:03Z, `approved-assets-preflight` 2026-07-31T08:13Z, `approved-assets-import` 2026-07-31T08:14Z), each with only a `branch_policy` protection rule (no required reviewers). `approved-assets-preflight`/`approved-assets-import` are now configured with variables `GOOGLE_CLOUD_PROJECT`, `GOOGLE_DRIVE_SCOPE`, `GOOGLE_WORKSPACE_ADMIN` and secrets `APPROVED_ASSETS_BASE_IDEMPOTENCY_KEY`, `APPROVED_ASSETS_DATABASE_URL`, `CANONICAL_40_CONTENT_FOLDER_ID`, `GOOGLE_SERVICE_ACCOUNT_JSON_B64` (names only, no values read); `approved-assets-migrations` has only `APPROVED_ASSETS_DATABASE_URL`. No `Migrate Approved Assets` workflow run exists in Actions history — still zero dispatches. Two of the four rollout-authorization PRs merged 2026-08-03 (`#95`, `#97`); `#93` and `#94` remain **open, unmerged** (`#93` is CONFLICTING pending rebase onto post-`#95`/`#97` main; `#94` is stacked on `#93`) to replace the missing-Enterprise Required-Reviewers assumption with an exact-subject authorization receipt before any phase may be dispatched. | Do not dispatch even a zero-write phase. Rebase and merge the open exact-subject rollout-authorization PRs (`#93`, `#94`); then complete the pgBackRest backup/restore acceptance (rehearsal now green — see Postgres row — but production `adapteng_ops` still has no stanza/schedule configured) before preflight and reviewed writes. |
...
| GitHub repository controls | **Live as of 2026-08-02T15:30 CEST, re-verified via read-only ruleset API on 2026-08-03:** an active `main-protected` repository ruleset now exists on all five active repositories — `adapteng-company-os` (id `20236724`), `adapteng-automation-platform` (`20236725`), `ai-dev-loop-control-plane` (`20236728`), `adapteng-marketing` (`20236729`), `adapteng-website` (`20236726`) — each blocking branch deletion and non-fast-forward pushes and requiring a pull request (0 required approving reviews, no code-owner/last-push requirement, thread resolution not enforced) plus repository-specific required status checks (e.g. `Python unit tests` + `Sensitive-reference validation` for Company OS, `validate` for the website). `current_user_can_bypass` is `never`. Legacy `PalinaRuban/adapteng` cannot be re-checked via the rulesets API (403: private repo below GitHub Pro), so its protection state is unverified this cycle. The five active repositories remain in a personal namespace; company ownership is still not evidenced. | Add required conversation-thread resolution if desired (currently off); confirm company ownership/transfer path. Separately isolate the shared deploy key, deliberately close stale PRs, and re-verify legacy-repo protection once a non-API (UI) check is available. |
...
| AI agent | **REJECT_LIVE, but AG-008 is now repository-merged (re-verified read-only 2026-08-03).** Control-plane main advanced from `affe6ea1e4d522be0df0641e98a08e20a84549ae` to `edadb09125f7fb5d173d5f595181d1384050b6b5` via PR #38 (`c6a5b5090acb2712ed2fb388a3170f9053f417b6`, merged 2026-07-30) and PR #39 (`edadb091...`, merged 2026-08-02). PR #38 requires a schema-valid `BusinessTaskEnvelope` before completion, enforces `no_external_action: true` with recursive rejection of approval/publish/send/action/execute fields and IDs, and makes `ModelGateway` reservation/finalization atomic and cap-bounded — closing the three P0 bypasses the 2026-07-26 audit reproduced (211-test AG-008 suite + 446-test repo suite passing). PR #39 then found and fixed a related gap the general JSON validator (not the artifact/envelope decoders) still had — silent acceptance of duplicate JSON keys and `NaN`/`Infinity` in `agent/LOOP_STATE.json`, project profiles and reviewer verdicts — with 28 targeted tests and a 673-test full-suite pass. `agent/NEXT_TASK.md` on main self-declares AG-008 `status: done`; CI on `edadb091` is green (`gitleaks`, `python-tests` ubuntu/windows, `repository-gates`, `business-eval-harness`, `container-acceptance` all success). No independent third-party review of this exact head is recorded. This closes the deterministic control-plane half only: no business worker, real EU Vertex provider or Drive runtime exists yet, canonical AI Gateway/migration 005 remain repo-only, and no live model call has occurred. Exact proof input `ART-2026-001`/`SRC-2026-001` remains selected and separate from the blocked CASE media path. Official Google docs re-verified `gemini-3.1-flash-lite` as GA in EU multi-region (`eu`, `https://aiplatform.eu.rep.googleapis.com`); Standard non-global rates are $0.275/M text-image-video input, $0.55/M audio input, $0.0275/M cached text input and $1.65/M text output/reasoning ($0.0121 for the 20k+4k reference before FX). | Do not present this as deployed/working AI. Independently re-review AG-008 at `edadb091` against the original four failures; then deploy persistent Postgres cost reservation/reconciliation, the EU Vertex adapter, Drive adapters, orchestration, canonical approval and runtime; ratify `AG-007`, verify Vertex IAM/live endpoint access plus ZDR/cache-off/FX, and keep hard caps at €0.10/call, €1/day and €10/month before the exact inactive proof. |
