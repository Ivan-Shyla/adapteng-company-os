...
runtime by itself. `adapteng-automation-platform` has merged the persistent
Postgres cost reservation/reconciliation, EU Vertex adapter and
deployment/readiness components. Company OS PR #232
(`b71b5602495f97e5cd229200254ab74d992e6b94`) records the AI Gateway as
live-verified on the private network, while Drive/orchestration/approval
composition and a model-backed business proof remain unaccepted. This is
accepted infrastructure readiness, **not accepted/working business AI**.
...
| `AG-008` | ai-dev-loop-control-plane | Harden production completion boundary | **Repository implementation merged, not independently re-reviewed:** PR #38 (`c6a5b5090acb2712ed2fb388a3170f9053f417b6`, merged 2026-07-30) requires a schema-valid `BusinessTaskEnvelope`, enforces `no_external_action: true` with recursive rejection of approval/publish/send/action/execute fields and IDs, and makes `ModelGateway` cost accounting atomic and cap-bounded. PR #39 (`edadb09125f7fb5d173d5f595181d1384050b6b5`, merged 2026-08-02) closes the related duplicate-key/non-finite JSON validation gap. PR #40 (`327fc4b63ec60afc8a8a6c3169d062a58d9eb4da`, merged 2026-08-03) records the first two P0 findings closed and durable cross-process spend/latch authority still delegated to the platform. Automation-platform has since merged that persistent authority and Company OS records live internal gateway readiness, but the representative quality set is unratified and no model-backed business proof is accepted. |

### 10.5 Days 30–60: first business AI pilot

| ID | Repository/system | Work | Definition of done |
|---|---|---|---|
| `AI-001` | marketing/control-plane | Content & Case Draft skill | Inputs/outputs follow schemas. **Repository implementation merged:** marketing PR #19 (`5b9af0e128e19a05208099d10a822daa0d92467e`) and hardening PR #20 (`9afaf96db1024685652383bbf825fc2994da13bc`) provide deterministic, offline, draft-only behavior and retire the former live-capable media/WordPress entrypoints before I/O. The two firsts are explicit: `CASE-2026-001` is the first governed raw-source/case migration plus evidence-bounded deterministic case draft, not the live model proof; exact public package `ART-2026-001` using `SRC-2026-001` is selected for the first live model-backed Company Drive proof. All four founder content units remain unratified; CASE media/publication remain blocked pending source reconciliation. Remaining live gates: governed Company Drive write path, `AG-007` ratification, EU Vertex privacy/cache/FX proof and the measured inactive model call. |
...
| Component | Verified status (through 2026-09-07 unless a row cites a later date) | Next milestone / constraint |
|---|---|---|
| Overall stage | **Operational foundation + controlled migration.** Company-owned Baserow, the Shared Drive folder skeleton, Postgres, the internal adapter and the two standing governed self-hosted workflows are live. Company OS PR #230 (`c741bb32294bc3987c112781702e4ae41afd6c38`) records the bounded L1 read-only flow as **OPERATIONAL** after five live checks passed with zero writes and the proof workflow returned inactive. PR #232 (`b71b5602495f97e5cd229200254ab74d992e6b94`) records the AI Gateway as **live-internal-verified** after private-network `/healthz` and database-backed `/ready` both returned 200. These are accepted infrastructure/L1 facts, not a live business-artifact pilot: no governed Drive import or model call is evidenced. The isolated WEB-002 T1–T4 preflight is accepted by PR #235, but production delivery is not cut over. **All nine logical migrations remain applied and must not be replayed.** | Complete the production backup gate and the explicit WEB-002 configuration/reconciliation before cutover. Keep approved-source import, model-backed proof and the separate `CASE-2026-001` media lane gated; never infer those outcomes from merge, CI, deployment or L1 acceptance. |
| Weekly repository delta | **Merged implementation:** automation-platform `main` is `9957271a267ececd41abd09c22243d35fe87416f`; PR #130 added the bounded read endpoints and PR #131 corrected WEB-002 evidence-lane validation. Website `main` is `cd17a2969041ed68cb336150182df544c73c6a46`; PRs #179–#186 rebuilt, published and production-QA'd the Expertise pages, while PR #187 pins the corrected WEB-002 artifact. **Live/accepted where explicitly evidenced:** Company OS PRs #230/#232 accept L1 and AI Gateway internal readiness; website PRs #181/#183–#186 record the three-locale Expertise publication and subsequent production QA, while native CZ/RU review remains pending; Company OS PR #235 accepts only the isolated WEB-002 preflight. **Not cut over / not run:** no `configure-lead-intake` run, governed Drive import or model-backed business call is evidenced. Control-plane remains `327fc4b63ec60afc8a8a6c3169d062a58d9eb4da`; marketing remains `9afaf96db1024685652383bbf825fc2994da13bc`, with offline AI-001 and retired former provider entrypoints. No actual recurring-cost evidence changed. | Owner: Ivan. Resolve the production backup prefix/configuration gate, then run the separately authorized WEB-002 configuration and reconciliation. Treat the three repository-only AI/content surfaces as non-live until their own acceptance gates pass. |
...
| Governed Baserow adapter | Existing write service remains live internal-only, bearer-authenticated and field-ownership proven; no public FQDN. **Merged repository implementation:** automation PR #130 (`f9daf1b50c490e4fdaa4a36cc38beddf18c022ac`) adds `system`-only read endpoints with a strict field allowlist and sample cap 1–5. **Live/accepted L1:** automation PR #129 (`1ba4c65ccd26a4a99b6c2c5861aa31ca66d76b2c`) and Company OS PR #230 record a successful self-hosted proof: health, authorized schema/sample, missing-auth denial and disallowed-kind denial all passed; three allowlisted records, zero writes; workflow inactive afterward. | Keep L1 read-only and human-reviewed. Populate/reconcile the non-blocking monitoring fields before depending on them; keep approved-source import Baserow writes at zero. |
...
| Website | `adapteng.com` remains the live Cloudways site. WEB-001 repository implementation remains merged at `6770e749fbf1345bfe15f260e574da93fa4df329`; a later plugin deployment does not itself establish delivery-mode cutover. Website `main` is `cd17a2969041ed68cb336150182df544c73c6a46`. The v2.1 runtime, isolated preview CI and manual exact-current-`main` deployment guards are merged implementation; Playground is not hosted staging. **Explicit live evidence:** the Expertise rebuild was published in all three locales under owner approval and subsequently corrected/QA'd in PRs #181–#186; exact-SHA theme deploy run `34056864162` and plugin deploy run `34055802416` succeeded. Those runs do not establish WEB-002 delivery mode. | Preserve the manual exact-main deployment gates. Native CZ/RU review and website backup/restore evidence remain open. Keep WEB-002 production cutover separate from content publication; retire MM-18 last. |
| Website weekly acceptance | **Live/accepted:** PR #175 (`86bb3b1822cbd5fba70f85cd63656a0d4188266b`) records all nine production forms verified; PRs #181/#183–#186 record owner-approved three-locale Expertise publication plus production corrections/QA through deployed theme SHA `31b4e30f56a6d3c257116bc4fe5a816fc1629843`. **Accepted preflight, not cutover:** run `34058980920`, automation PR #131 and website PR #187 passed the isolated WEB-002 T1–T4 lane with one row per effect key, zero production writes and final inactive state. No subsequent `configure-lead-intake.yml` run is recorded, so production cutover and delivery mode are not accepted by that evidence. **Blocked:** the website repository still marks backup/restore as not tested and native CZ/RU review remains pending. | Close the production backup gate, then separately authorize configuration, producer cutover and seven-day reconciliation. Record delivery mode and post-cutover acceptance explicitly; do not derive either from plugin/theme deployment. |
...
| AI agent | **AI Gateway infrastructure live-verified; business AI not accepted.** Automation-platform PRs #109/#112/#113/#114/#127 merged fail-closed credential, bind, readiness and deployment contracts. Company OS PR #232 records a 2026-09-06 private-network proof against `http://ai-gateway:8081`: `/healthz` 200, database-backed `/ready` 200, name resolved and zero writes. This explicitly closes current container/network/database readiness despite older automation repository text saying not deployed. It does not exercise the EU Vertex client, Drive adapters, orchestration or approval composition. Control-plane `main` remains `327fc4b63ec60afc8a8a6c3169d062a58d9eb4da`; its representative quality set remains synthetic/unratified. No live model call or incurred AI cost is evidenced. | Run only the separately governed, schema-valid, side-effect-free first model proof after Drive, approval, privacy/cache/FX and quality gates pass. Preserve the already-applied migrations and durable cost authority; infrastructure readiness is not business-output acceptance. |
...
| Backup/security | The disposable encrypted pgBackRest/WAL restore contour remains proven by run `30811383748`, but production `adapteng_ops` still has no accepted stanza/schedule/retention/monitoring proof. Company OS PR #235 records the current fail-closed blocker: the configured repository prefix uses an underscore while `scripts/postgres_restore_guard.py` pins the hyphenated `/adapteng-ops`; no backup exists under either prefix. Open draft PR #222 adds repository regression coverage but is not merged and cannot change the provider variable. The Baserow token incident and owner-side rotations remain open; current tracked files are sanitized. | Owner: Ivan. Align the provider variable to the guarded hyphenated prefix, align lifecycle/key scope, then produce the separately reviewed production backup/restore evidence before WEB-002 cutover. Revoke/rotate compromised or launch-time credentials and verify old values fail; do not rewrite history. |
...
ready and L1/platform readiness is accepted, but personal-Drive workflow cutover,
first corporate draft, business AI pilot, production backup configuration,
WEB-002 production cutover and owner recovery acceptance remain open.
...
live systems and never auto-merges. Its scheduled repository-status runs are
recorded through run `33200723380` (2026-08-28); successful execution does not
itself establish any live-system state.
