...
| AI agent | **REJECT_LIVE until the remaining runtime gates pass; deterministic components and the owner-dispatch readiness fallback are merged.** Control-plane `main` is `327fc4b63ec60afc8a8a6c3169d062a58d9eb4da`; automation-platform `main` is `0794212ea74a45095a45b9e1046cf5f8dc89f946`. No live model call has occurred. The exact proof input remains `ART-2026-001` / `SRC-2026-001`, separate from blocked CASE media. Owner authorization 2026-08-06 selects the existing exact identity `adapteng-ai-operator@adapteng-workspace-automation.iam.gserviceaccount.com` and permits the reviewed main-only owner-dispatch fallback for Vertex readiness and the eventual first live run; Environment secret values remain non-readable and environment-scoped. The readiness workflow has not yet passed, AI Gateway migrations remain unapplied, approved-source import/replay has not run, and local proof-runtime configuration has not been verified. | Run read-only Vertex IAM/model-metadata readiness for project `adapteng-workspace-automation`, location `eu`, model `gemini-3.1-flash-lite`; then complete the governed migrations and approved-source import/replay, run the zero-write proof preflight, and permit exactly one capped `generateContent` call only after every prerequisite independently verifies. |
