...
- Full departure from personal Drive is **not complete**: current MM
  ...
  blocked pending live Sheet-vs-Git reconciliation. Corporate copy and workflow
  rewiring are in progress. Owner Manager/recovery and break-glass acceptance
  remain open.
...
|---|---|---|
| Overall stage | **Operational foundation + controlled migration.** Company-owned Baserow, Shared Drive, Postgres, internal adapter and two governed self-hosted workflows are live. The daily content/media path and business AI are not yet cut over. | Finish the governed Drive copy/write path; run the exact `ART-2026-001`/`SRC-2026-001` inactive model proof after its remaining gates; migrate `CASE-2026-001` on its separate governed lane without unblocking media/publication. |
...
| First governed raw-source migration | `CASE-2026-001` inventoried read-only: intake marker, case note, 4 HEIC images and 2 MOV videos. Its role is the first governed raw-source/case migration plus evidence-bounded deterministic case draft, **not** the first live model proof. The original is untouched; all media and publication remain fail-closed pending live Sheet-vs-Git reconciliation. Bounded claims/style/red-lines are in `ai/ai-001-pilot-intake.md`. | Copy through the company SA into corporate `00_Case_Uploads`; verify replay/no duplicates; create Baserow/Drive pointers only through a replay-safe reserved business ID. Do not promote media or publication records until human reconciliation. |
...
| Platform governance evidence | Current live authority remains the evidence in this table: Company Shared Drive and the Baserow skeleton are live; migrations 001/004 are applied; the internal adapter plus AUT-001/WEB-002 are live and proven; n8n Cloud is 89 non-archived / 33 active. A repository-only audit does not override that live evidence. | Still open: company-owned GitHub namespace and branch-protection evidence; stale PR cleanup; workflow-to-credential binding isolation and second-admin continuity; base vendor cost/renewal/SLA evidence; shared deploy-key blast-radius reduction; media implementation consolidation; and live rollout of repo-only migrations 002/003/005/006/007. |
