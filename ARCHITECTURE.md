...
| `AUT-001` | automation-platform | Versioned Baserow adapter | Upsert by stable ID is idempotent; patches only workflow-owned fields (§3.4). **Repository implementation merged:** adapter library PR #59; read-only live binding PR #77 (`6e46d6fd89d1e3efaff00dac58cd8d5b55d0c8e3`) captured and committed the `Company Operations` table ids (workspace `AdaptEng OS` id 161 → database id 260; ORGANIZATION=842, PERSON=843, OPPORTUNITY=844, PROJECT/CASE=845, ACTION=846, DOCUMENT=847, CONTENT/CONTENT_GROUP=848, SYSTEM=849) with a field-parity report showing zero missing live fields. **Built, verified, not merged:** PR #78 wraps the library in an authenticated internal HTTP service (`POST /v1/upsert` + `/healthz`, constant-time bearer, fail-closed) plus a guarded, idempotent on-start migration (`RUN_MIGRATIONS_ON_START`) that applies the drift-tested stable-id allocator DDL on first deploy; 54 mock-only tests, `validate_repo` and secret-scan pass locally. Live Baserow use remains gated (token, backup, deploy, n8n canary). |
...
| `automation-platform` repository | `main` at `6e46d6fd89d1e3efaff00dac58cd8d5b55d0c8e3`; PRs #58/#68/#69/#70/#71/#72/#74/#75/#76/#77 merged (COS-004 dispatch-only provisioning workflow and the AUT-001 read-only `Company Operations` binding are now on `main`); PR #78 (AUT-001 HTTP service + guarded on-start migration, head `983fd12`) is complete and verified locally but not merged. **CI is currently blocked account-wide:** since 2026-07-25 ~11:54Z every workflow fails to start in ~3s with "recent account payments have failed or your spending limit needs to be increased" (a fresh GitHub Actions billing/spending-limit lapse — the prior 11:49Z runs were green), so no PR can show green checks until it is restored | **Owner: Ivan** — restore GitHub Actions billing / raise the spending limit, then re-run CI on PR #78 head `983fd12` and merge on green. Repository evidence does not imply live migration, workflow import, deployment, model call or remediation; complete the component-specific gates below |
