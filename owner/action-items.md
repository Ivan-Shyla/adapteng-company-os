...
## Governed agent pilot — DONE 2026-09-14

Everything in this section through the 2026-09-08 "exact sequence" (items
1-10 below) is now complete and live-verified. Summary, full detail in
`registry/services.yaml`'s `adapteng-agent-runtime` and
`adapteng-drive-adapter` entries (`canary_evidence` blocks):

- `VERTEX_SERVICE_ACCOUNT_JSON` installed; the Vertex/Gemini identity is bound and working.
- `agent-runtime` and `drive-adapter` both deployed (Coolify, private network only) and `running:healthy`, each on its own least-privilege Postgres role (`scripts/governed_runtime_roles.py`).
...
| # | Priority | Provider / screen | Name or setting | Exact action | Minimum scope | Verification | Blocks |
|---|---|---|---|---|---|---|---|
| 7 | 🟠 | Hetzner host shell (SSH) → Coolify containers | Coolify scheduler / queue worker | Restart Coolify's scheduler so timed work runs again. Nothing scheduled has fired since 2026-08-27 although the API, deployments and every workload are healthy, so this is invisible from the dashboard. While it is down, no Coolify schedule of any kind runs — backups, Docker cleanup, or anything added later. Check free disk while you are on the host: cleanup is configured `0 0 * * *` at an 80% threshold and has not applied those terms in twelve days, and `high_disk_usage_notification_sent` reads `None` because that flag is itself set by a scheduled check. A quiet disk alarm and a dead one look identical from here. | The Coolify control-plane containers only; no application restarts | Dispatch **Coolify deploy** with `operation=scheduler-check` and see `RESULT scheduler-check alive`; then delete `.github/workflows/backup-daily.yml` | Every current and future Coolify schedule, and disk-pressure alerting |
