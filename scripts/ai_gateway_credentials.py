...
def operate_mint_additional_caller(client: driver.Client, caller_token: str) -> int:
    """Add one caller token to the allowlist, preserving every token already there.

    mint-caller overwrites CALLER_KEY outright -- correct the first time this
    application is ever configured, wrong here: model-smoke's own caller
    (ae.smk) already depends on the existing value, and this repository's own
    coolify_deploy.py model-smoke tests prove that credential is live and
    working. Clobbering it to add a second caller would break a proven path
    to fix nothing.

    ``caller_token`` is generated by the caller of this script (a company-os
    repository secret set directly, not generated here), so this operation
    never invents a value on its own -- it only appends the one it was given.
    services/ai-gateway/app/auth.py's parse_token_list already treats
    CALLER_KEY as a comma-separated list, so appending is the native format,
    not a workaround.
    """

    driver.emit(f"--- mint-additional-caller {RESOURCE_NAME}")
    uuid = locate(client)
    driver.emit(f"    application {RESOURCE_NAME}: uuid={uuid}")
    ...
    existing_raw = None
    for entry in entries:
        if isinstance(entry, dict) and entry.get("key") == CALLER_KEY:
            existing_raw = entry.get("value")
            break
    if existing_raw:
        driver.register_redaction(existing_raw)
    existing_tokens = [part.strip() for part in (existing_raw or "").split(",") if part.strip()]
    if not existing_tokens:
        raise driver.Abort(
            f"{CALLER_KEY} is not currently set on {RESOURCE_NAME}; use mint-caller "
            "for first-time setup, not this additive operation"
        )
    driver.emit(f"    existing tokens: {len(existing_tokens)}")

    if caller_token in existing_tokens:
        driver.emit("    the given token is already present; nothing to add")
        driver.emit(f"RESULT mint-additional-caller ok already present, {len(existing_tokens)} tokens")
        return 0

    combined = ",".join(existing_tokens + [caller_token])
    driver.register_redaction(combined)
    write_environment_value(client, uuid, CALLER_KEY, combined)

    entries_after = driver.read_environment_entries(client, uuid)
    after_raw = None
    for entry in entries_after:
        if isinstance(entry, dict) and entry.get("key") == CALLER_KEY:
            after_raw = entry.get("value")
            break
    after_tokens = [part.strip() for part in (after_raw or "").split(",") if part.strip()]
    missing_prior = set(existing_tokens) - set(after_tokens)
    if missing_prior:
        raise driver.Abort(
            f"{CALLER_KEY} was written but no longer contains every prior token "
            f"({len(missing_prior)} missing); refusing to report success on a write "
            "that may have dropped the working ae.smk credential"
        )
    if caller_token not in after_tokens:
        raise driver.Abort(
            f"{CALLER_KEY} was written but does not contain the new token; "
            "the write may not have taken effect"
        )
    driver.emit(f"    env {CALLER_KEY}: {len(after_tokens)} tokens, all prior tokens preserved")
    driver.emit("")
    driver.emit("    the combined value is held only by Coolify; it is not printed.")
    driver.emit(f"RESULT mint-additional-caller ok {len(existing_tokens)} preserved, 1 added")
    return 0
...
def parse_arguments(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "operation",
        choices=("bind-adc", "mint-caller", "mint-additional-caller", "status"),
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if arguments.operation == "mint-additional-caller":
            caller_token = os.environ.get("GOVERNED_AGENT_RUNTIME_GATEWAY_TOKEN", "")
            if not caller_token:
                raise driver.Abort(
                    "no GOVERNED_AGENT_RUNTIME_GATEWAY_TOKEN was supplied, so there is "
                    "nothing to add. This operation appends a token it is given; it "
                    "never generates one itself."
                )
            driver.register_redaction(caller_token)
            return operate_mint_additional_caller(client, caller_token)
